...
SYSTEM_INSTRUCTIONS = (
    ...
    # NEW PATIENT FLOW -- THE CRITICAL FIX
...
async def handle_function_call(function_name, arguments, call_id, session, openai_ws):
    ...
    try:
        ...
        # == BOOKING =====================================================
            ...
            if not session.get("verified") or not session.get("patient_data"):
                result = {"status": "ERROR", "message": "Patient must be verified first. Call create_new_patient() or verify_existing_patient() before booking."}
            else:
        ...
        # == COMPLAINT ===================================================
        # Local vars for first_name/last_name avoid f-string quote issues
    ...
    # response.create sent ONCE here. NOT in response.done. NOT in watchdog.
    # -----------------------------------------------------------------------
...
#     if not host:
#         return HTMLResponse("Host header missing", status_code=400)
...
async def handle_media_stream(websocket: WebSocket):
    ...
    try:
        ...
        #     try:
        #         await asyncio.sleep(INTERRUPTION_CONFIRMATION_MS / 1000.0)

        #         if session and session.get("is_speaking") and session.get("interruption_pending"):
        #             print("[INTERRUPT] Stopping bot")

        #             # Let last audio frames flush
        ...
        async def receive_from_openai():
            ...
            try:
                async for message in openai_ws:
                    ...
                    #         # 1. Stop OpenAI audio generation immediately
                    #         try:
                    #             await openai_ws.send(json.dumps({"type": "response.cancel"}))
                    #         except Exception as e:
                    #             print(f"[BARGE-IN ERROR] Cancel response failed: {e}")

                    #         # 2. Flush Twilio audio buffer immediately (caller hears silence)
                    #         if stream_sid:
                    #             try:
                    #                 await websocket.send_json({"event": "clear", "streamSid": stream_sid})
                    #             except Exception as e:
                    #                 print(f"[BARGE-IN ERROR] Twilio clear failed: {e}")

                    #         # 3. Reset speaking state
                    #         session["is_speaking"] = False
                    #         session["interruption_pending"] = False

                    #         # 4. Cancel any pending interruption timers
                    #         if interrup_task:
...
if __name__ == "__main__":
    ...
    print(f"  Fixes    : mandatory create_new_patient | no echo-duplicates | no false biz logs")
    print("=" * 70)
    uvicorn.run(
        "main:app",
        host="0.0.0.0",
