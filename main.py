...
VAD_THRESHOLD            = 0.75
PREFIX_PADDING_MS        = 300
SILENCE_DURATION_MS      = 700   # FIX C: lower for faster barge-in response
CLOUD_RUN_WSS_BASE       = "wss://green-diods-dental-clinic-production.up.railway.app"
...
SYSTEM_INSTRUCTIONS = (
    ...
    # ── FIX B: Treatment answer rules ────────────────────────────────────────
    "TREATMENT ANSWER RULES — NEVER BREAK:\n"
    "1. ONLY recommend or mention treatments that are in the 18-item SERVICES list above.\n"
    ...
    "5. If the knowledge base has no answer, say: 'I don't have that detail right now —\n"
    "   our team can help you with that in clinic.'\n"
    ...
    # ── FIX A: Do NOT ask new/existing upfront. Wait for user intent first. ──────
    "CALL START BEHAVIOUR:\n"
    "After the greeting, STOP SPEAKING and WAIT silently for the caller to tell you what they need.\n"
    "Do NOT ask 'Are you an existing or new patient?' immediately after the greeting.\n"
    "Listen to their intent first, then decide what to do.\n\n"

    "STEP 0 — WHEN TO ASK 'Are you an existing patient or a new patient?':\n"
    "Ask this question ONLY when the caller's intent requires it:\n"
    "  MUST ASK -> when caller wants to: BOOK an appointment, UPDATE an appointment, CANCEL an appointment\n"
    "  MUST ASK -> when caller wants: their order status, upcoming appointments, treatment history\n"
    "  DO NOT ASK -> for: clinic address, hours, phone number, services list, pricing, payment,\n"
    "                     insurance questions, warranty questions, general dental questions,\n"
    "                     complaints (complaints have their own verification rules below),\n"
    "                     ANY general question — answer directly without asking\n\n"
    "  EXCEPTION — UPDATE or CANCEL: skip Step 0 entirely.\n"
    "   Only existing patients have appointments.\n"
    "   Go directly to EXISTING PATIENT FLOW (last name -> DOB -> verify).\n\n"
            ...
            async for message in openai_ws:
                ...
                try:
                    if event_type == "session.updated":
                        ...
                        if session:  # FIX C: clear so barge-in guard window closes properly
                            session["current_response_id"] = None

                    elif event_type == "input_audio_buffer.speech_started":
                        # FIX C: interrupt if bot is speaking OR has an active response in-flight
                        _should_interrupt = session and (
                        ...
                        if _should_interrupt:
                            ...
                            session["audio_queue"].clear()
                            session["last_assistant_item_id"] = None
                            session["current_response_id"]    = None  # FIX C
                            session["audio_start_time"]       = None
                            session["elapsed_ms"]             = 0
                            session["is_speaking"]            = False
                        else:
